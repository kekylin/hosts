...
- **智能评分系统**（8 线程并发）：
  ...
  - **评分规则**：
    - 功能分：HTTPS成功=100分，TCP成功=50分，Ping成功=25分
    - 延迟惩罚：ping延迟/10（有ping数据）或固定15分（无ping数据）
    - 综合分数：`功能分 - 延迟惩罚`
  - **智能排序**：按分数从高到低排序，每种协议取前 N 个最优 IP（N=MAX_IPS）
...
- **自动化更新**：GitHub Actions 每天定时运行一次（北京时间 00:00，默认双栈模式、每协议最多 1 个 IP）
